...
inline std::string to_string(const utils::optional<xdr::transducer_info> & data)
{
	...
	return src::nmea::to_string(value.transducer_type) + ","
		+ src::nmea::to_string(value.measurement_data) + ","
		+ src::nmea::to_string(value.units_of_measurement) + ","
		+ src::nmea::to_string(value.name);
}
}
